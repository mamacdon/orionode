...
function cover(modulePath, args) {
    var cover = path.relative(path.dirname(modulePath), path.resolve('node_modules/.bin/cover')),
        child;

    args.unshift(cover);
    // FIXME: is CWD guaranteed to be the server root?
	child = child_process.spawn(PATH_TO_NODE, args, {
	    cwd: path.dirname(modulePath),
	    stdio: [ null, process.stdout, process.stderr ] // FIXME: dumping child output to console for debug...
	});
	
	return child;
}

function generateCoverageReport(modulePath) {
    var moduleDir = path.dirname(modulePath),
        oldReport = path.resolve(moduleDir, 'cover_html'),
        report = cover(modulePath, [ 'report', 'html' ]);
    
    async.waterfall([
        function (next) {
            fs.exists(oldReport, function (exists) {
                next(null, exists);
            });
        },
        function (reportExists, next) {
            if (reportExists) {
                // Delete old coverage report.
                fileUtil.rumRuff(oldReport, next);
            } else {
                next();
            }
        },
        function (next) {
            report.on('exit', function (code) {
                code == 0 ? next(null) :
                    next(new Error("cover exited with errors."));
            });
        },
        function (next) {
            // Delete coverage data from module run.
            fileUtil.rumRuff(path.resolve(moduleDir, '.coverage_data'), next);
        }
    ], function (error) {
        if (error) {
            console.error("Failed to generate coverage report. Error was:", error.message);
            return;
        }
        
        console.log("Coverage report is in 'cover_html'.");
    });
}

function runWithCoverage(modulePath, args) {
    args = Array.isArray(args) ? args : [];
    args.unshift('run', modulePath, '--');
    
    return cover(modulePath, args);
}
...
module.exports = function(options) {
	...
	function coverApp(req, res, modulePath, args, debugInfo) {
		var child, app;
		try {
		    child = runWithCoverage(modulePath, args);
			var location = url.resolve(url.format({
				protocol: req.protocol, // TODO this may be bad
				host: req.headers.host,
				pathname: nodeRoot + '/'
			...
			appTable.put(child.pid, app);
			child.on('exit', function() {
			    generateCoverageReport(modulePath);
				console.log('exit # ' + child.pid);
				appTable.remove(child.pid);
			});
			write(201, res, { Location: location }, app.toJson());
	...
	return resource(nodeRoot, {
		...
		POST: function(req, res, next, rest) {
			...
			if (rest === 'start') {
			    ...
			    if (checkPath(modulePath) && checkArgs(args)) {
					coverApp(req, res, safeModulePath(modulePath), args);
				}
			} else if (rest === 'debug') {
				port = params.port;
