			...
			switch (rest) {
				...
				case 'debug':
					...
					if (typeof modulePath !== 'string') {
						...
						modulePath = safeModulePath(modulePath);
						var debugArg = (args && args.length > 0 ? args[0] : "5858");
						spawnChild(req, res, PATH_TO_NODE, modulePath, ["--debug-brk=" + debugArg].concat(modulePath));
					}
					break;
				case 'debug_inspect':
					var params = req.body;
					var args = params.args;
 					if (args && !Array.isArray(args)) {
						...
						var portArg = (args && args.length > 0 ? args[0] : "8082");
						spawnChild(req, res, PATH_TO_NODE, modulePath, [modulePath].concat("--web-port=" + portArg));
