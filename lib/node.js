...
//	var tempDir = options.tempDir;
	...
	function spawnChild(req, res, cmdFile, modulePath, args) {
		try {
			...
			switch (rest) {
				...
				case 'spawn':
					...
					if (typeof modulePath !== 'string') {
						...
						spawnChild(req, res, PATH_TO_NODE, modulePath, [modulePath].concat(args));
					}
					break;
				case 'debug':
					var params = req.body;
					var modulePath = params.modulePath, port = params.port;
					if (typeof modulePath !== 'string') {
						writeBadRequest(req, res, 'Missing parameter "modulePath"');
					} else if (typeof port !== 'number') {
						writeBadRequest(req, res, 'Parameter "port" must be a number');
					} else {
						modulePath = safeModulePath(modulePath);
						spawnChild(req, res, PATH_TO_NODE, modulePath, ["--debug-brk=" + port].concat(modulePath));
					}
					break;
				case 'debug_inspect':
					var params = req.body;
					var port = params.port;
 					if (typeof port !== 'number') {
						...
						var modulePath = require.resolve('node-inspector/bin/inspector');
						spawnChild(req, res, PATH_TO_NODE, modulePath, [modulePath].concat("--web-port=" + port));
